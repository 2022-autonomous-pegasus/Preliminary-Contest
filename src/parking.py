...
while not rospy.is_shutdown():
    ...
    distance = math.sqrt(int(arData["DX"])**2 + int(arData["DY"])**2)
    ...
    # DX값 DY값 Yaw값 구하기
    dx_dy_yaw = "X:"+str(int(arData["DX"]))+" DY:"+str(int(arData["DY"])) \
                +" Yaw:"+ str(round(yaw,1)) 
    ...
    theta=math.atan(arData["DX"]/arData["DY"])*180/math.pi #차량 정면으로 그은 선과 벽면에 수직으로 내린 선 사이의 각도를 theta로 지정
    print("Dx:%f Dy:%f Yaw: %f theta: %f"%(arData["DX"],arData["DY"],yaw,theta))
    
    # 핸들 조향각 angle값 설정하기
    #2차원 평면에서 차량의 조향각은 theta+yaw값이 되고 회전 방향을 양수와 음수로 고려했을 때 yaw값은 내가 원하는 방향의 반대 부호를 가지게 됨으로 yaw값의 부호를 바꾸어 주게 되면 조향각을 계산할 수 있다.
    if theta-yaw>0: 
        angle=theta-yaw
    elif theta-yaw<0:
        angle=theta-yaw
    ...
    # 차량의 속도 speed값 설정하기
    if  distance>71: #벽면과 차량 사이의 최소값을 71로 설정
        speed= 40 #거리가 71 이상일 경우 속도 40으로 주행
    else:# 거리가 71일 경우 속도 0으로 정지
        speed=0	
